...
export const Navbar = ({ children, className }: NavbarProps) => {
  ...
  return (
    <motion.div
      ...
      className={cn("fixed inset-x-0 top-4 z-40 w-screen", className)}
    >
  ...
  return (
    ...
    >
      {items.map((item, idx) => (
        ...
        <NavLink
          className={cn("relative px-4 py-2 text-background hover:bg-linear-to-r hover:text-transparent text-base bg-clip-text transition-all duration-300", blueGradientClass)}
          key={`link-${idx}`}
          to={item.href}
        >
...
export const NavbarLogo = () => {
  return (
    ...
    >
      <Acurve />
      <span className="font-medium text-background">Acurve</span>
    </NavLink>
  );
};
